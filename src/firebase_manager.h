...
class FirebaseManager {
  ...
  public:
    FirebaseManager(String mac) {
      ...
      antiSpanTimer = millis()-ANTI_SPAN_MAX_TIME;
    ...
    bool sendDataToDB(FirebaseJson json) {
        Serial.println(" Push new data to DB.");
        String path = DB_DEVICES_DATA + deviceMAC + DB_SENSOR_VALUES;
    ...
    void sendNotification(String alertMsg) {
      if(millis() - antiSpanTimer < ANTI_SPAN_MAX_TIME)
        return;
      antiSpanTimer = millis();
      ...
      msg.targets.to = "/topics/" + FCM_TOPIC_NAME_PREFIX + deviceMAC;

      // msg.options.time_to_live = "1000";
      msg.options.priority = "high";

      msg.payloads.notification.title = FCM_NOTIF_TITLE;
      msg.payloads.notification.body = alertMsg;
      //msg.payloads.notification.icon = "myicon";
      //msg.payloads.notification.click_action = "FLUTTER_NOTIFICATION_CLICK";
      msg.payloads.notification.android_channel_id = "mobi_0km_iot_manager";


      FirebaseJson payload;

      // all data key-values should be string
      payload.add("alertMsg", alertMsg);
